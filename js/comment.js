...
import { request, HTTP_GET, HTTP_DELETE, HTTP_PUT } from './request.js';
import { configureSupabase } from './supabaseConfig.js';
...
export const comment = (() => {
    ...
    const comment = async() => {
        ...
        console.log('pagination getNext => getPer', pagination.getNext(), pagination.getPer() + pagination.getNext());

        return supabase.from('comment')
        .select('*').order('created_at', { ascending: false })
        .range(pagination.getNext(), pagination.getPer() + pagination.getNext())
            .then((res) => {
